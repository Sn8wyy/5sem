...
ETUApi::NowDay ETUApi::get_now_day() {
	...
	j = nlohmann::json::parse(ñlient.get_request("", URL));
...
std::string ETUApi::get_near_lesson(int group_number) {
	...
	std::string json = ñlient.get_request("", URL);
...
std::string ETUApi::get_lesson(nlohmann::json j) {
	std::string info;
	info += "Âðåìÿ: " + j["start_time"].dump().substr(1, j["start_time"].dump().size() - 2) + '\n';
	info += "Äèñöèïëèíà: " + j["name"].dump().substr(1, j["name"].dump().size() - 2) + '\n';
	info += "Òèï: " + lesson_type.at(j["subjectType"].dump().substr(1, j["subjectType"].dump().size() - 2)) + '\n';
	info += "Ïðåïîäàâàòåëü: " + j["teacher"].dump().substr(1, j["teacher"].dump().size() - 2) + '\n';
	if(j["second_teacher"].dump() != "\"\"") 
		info += "Ïðåïîäàâàòåëü: " + j["second_teacher"].dump().substr(1, j["second_teacher"].dump().size() - 2) + '\n';
	info += "Àóäèòîðèÿ: " + j["room"].dump().substr(1, j["room"].dump().size() - 2) + '\n';
	info += "Íîìåð íåäåëè: " + j["week"].dump().substr(1, j["week"].dump().size() - 2) + '\n';
	info += '\n';
	return info;
}